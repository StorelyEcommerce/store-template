...
// .wrangler/tmp/bundle-SrYQvz/checked-fetch.js
...
// .wrangler/tmp/bundle-SrYQvz/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-SrYQvz/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-SrYQvz/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
app2.post("/:slug/checkout", async (c) => {
  ...
  for (const item of items) {
    const product = products.find((p) => p.id === item.productId);
    const images = [];
    if (product.imageUrl && !product.imageUrl.startsWith("data:") && product.imageUrl.length < 2048) {
...
app2.post("/:slug/orders/confirm", async (c) => {
  const store = c.get("store");
  const body = await c.req.json();
  if (!body.sessionId) {
