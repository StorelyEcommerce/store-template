...
// .wrangler/tmp/bundle-J5B13J/checked-fetch.js
...
// .wrangler/tmp/bundle-J5B13J/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-J5B13J/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-J5B13J/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
app2.get("/:slug/products", async (c) => {
  ...
  const transformed = products.results.map(toCamelCase);
  return c.json(transformed);
});
app2.get("/:slug/products/:id", async (c) => {
  const store = c.get("store");
  const id = c.req.param("id");
  const product = await c.env.DB.prepare("SELECT * FROM products WHERE store_id = ? AND id = ?").bind(store.id, id).first();
  if (!product) {
    throw new HTTPException(404, { message: "Product not found" });
  }
  return c.json(toCamelCase(product));
});
app2.post("/:slug/checkout", async (c) => {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    cancel_url: cancelUrl || `${c.req.header("origin")}/cart`,
