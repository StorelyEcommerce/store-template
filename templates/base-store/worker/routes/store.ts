...
app.get('/:slug/products', async (c) => {
    try {
        const store = c.get('store') as Store;
        if (!store || !store.id) {
            throw new HTTPException(500, { message: 'Store not properly resolved' });
        }
        
        // Check if DB is available
        if (!c.env.DB) {
            console.error('[STORE ROUTE] DB not available in environment');
        ...
        if (!products || !products.results) {
            console.error('[STORE ROUTE] Products query returned invalid result');
            return c.json([]);
        }
        ...
        console.error('[STORE ROUTE] Error fetching products:', err);
        console.error('[STORE ROUTE] Error details:', {
